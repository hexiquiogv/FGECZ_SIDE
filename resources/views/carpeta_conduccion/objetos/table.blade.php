  ...
  <tbody>
   @foreach($listados['objetos'] as $objeto)    
    <tr>
      ...
      <td>{{$objeto->MARCA.' '.$objeto->MODELO.' '.$objeto->COLOR}}</td>
      <td>
        <a class="btn btn-outline-primary btn-sm" href="#" onclick="javascript:fnObjeto('{{$objeto->id}}')" role="button">Ver</a>
        <a class="btn btn-outline-danger btn-sm" href="#" onclick="javascript:eliminarReload({{$objeto->id}},'cco')" role="button">Eliminar</a>
      </td>
    </tr>
   @endforeach
  </tbody>
