...
<!--   <div class="mb-3 col-sm-12 col-md-6 col-lg-4">
  ...
  <div class="mb-3 col-sm-12 col-md-6 col-lg-4">
    ...
    value="{{$imputado->TEMPORALIDAD}}" placeholder="xx años, xx meses">
  </div>
  <div class="mb-3 col-sm-12 col-md-6 col-lg-4">
    <label for="causa_H_reparacion_del_daño_" class="form-label">Tipo de reparación del daño:</label>
    <input type="text" class="form-control alfanum" name="causa_H_reparacion_del_daño_" id="causa_H_reparacion_del_daño_" 
    value="{{$imputado->REPARACION_DEL_DAÑO}}">
  </div>
  <div class="mb-3 col-sm-12 col-md-6 col-lg-4">
    ...
    value="{{$imputado->TEMPORALIDAD_SALIDAD_ALTERNAS}}">
  </div>
</div>
<script type="text/javascript">
  $("#frmCausasPenalesAI_{{$imputado->id}} #causa_H_acuerdo_reparatorio").change(function() {
    if (this.value==2) {$("#frmCausasPenalesAI_{{$imputado->id}} .fechaCump").show();}
    else
    {
      $("#frmCausasPenalesAI_{{$imputado->id}} .fechaCump").hide();
      $("#frmCausasPenalesAI_{{$imputado->id}} .fechaCump #causa_H_fecha_cumplimiento").val('');
    }
  });
  if ($("#frmCausasPenalesAI_{{$imputado->id}} #causa_H_acuerdo_reparatorio").val()==2) 
    {$("#frmCausasPenalesAI_{{$imputado->id}} .fechaCump").show();}
    else
    {
      $("#frmCausasPenalesAI_{{$imputado->id}} .fechaCump").hide();
      $("#frmCausasPenalesAI_{{$imputado->id}} .fechaCump #causa_H_fecha_cumplimiento").val('');
