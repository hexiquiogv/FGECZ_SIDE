   ...
   @foreach($listados['victimas'] as $victima)
    <tr>      
      ...
      @elseif($victima->TIPO_VICTIMA==7)
        <td>LA SALUD</td>
      @else
        <td>{{strtoupper($victima->NOMBRE_VICTIMA.' '.$victima->PRIMER_APELLIDO.' '.$victima->SEGUNDO_APELLIDO_VICTIMAS)}}</td>
      @endif
      <td>
        <a class="btn btn-outline-primary btn-sm" href="#" onclick="javascript:fnVictima('{{$victima->id}}')" role="button">Ver</a>
        <a class="btn btn-outline-danger btn-sm" href="#" onclick="javascript:eliminarReload({{$victima->id}},'dev')" role="button">Eliminar</a>
      </td>
    </tr>
   @endforeach
  </tbody>
