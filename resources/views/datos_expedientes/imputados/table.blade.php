  ...
  <tbody>
   @foreach($listados['imputados'] as $imputado)
    <tr>
      ...
      @if($imputado->TIPO_IMPUTADO==2)
        <td>{{strtoupper($imputado->RAZON_SOCIAL)}}</td>
      @elseif($imputado->TIPO_IMPUTADO==3)
        <td>LA SOCIEDAD</td>
      @elseif($imputado->TIPO_IMPUTADO==5)
        <td>SIN IDENTIFICAR/DESCONOCIDO</td>
      @elseif($imputado->TIPO_IMPUTADO==7)
        <td>LA SALUD</td>
      @else
        <td>{{strtoupper($imputado->NOMBRE_IMPUTADO.' '.$imputado->PRIMER_APELLIDO.' '.$imputado->SEGUNDO_APELLIDO_IMPUTADOS)}}</td>
      @endif            
      <td>
        <a class="btn btn-outline-primary btn-sm" href="#" onclick="javascript:fnImputado('{{$imputado->id}}')" role="button">Ver</a>
        <a class="btn btn-outline-danger btn-sm" href="#" onclick="javascript:eliminarReload({{$imputado->id}},'dei')" role="button">Eliminar</a>
      </td>
    </tr>
   @endforeach
  </tbody>
