  ...
  <div class="modal-dialog modal-fullscreen"><!--modal-dialog-scrollable modal-lg-->
    <div class="modal-content">
      ...
      <div class="modal-body">
        @include("datos_expedientes.objetos.form")
      </div>
  ...
  function saveObjeto(){
    ...
    function fnObjeto(idObjeto){ 
        ...
        success: function (result) {
            $('#objeto_objeto_1').html('<option value="-1">Seleccione una opción</option>');
            $.each(result.respuestas.objetos, function (key, value) {
                $("#objeto_objeto_1").append('<option value="' + value
                    .id + '">' + value.Valor + '</option>');
            });
            $('#objeto_objeto_2').html('<option value="-1">Seleccione una opción</option>');
            $.each(result.respuestas.objetos, function (key, value) {
                $("#objeto_objeto_2").append('<option value="' + value
                    .id + '">' + value.Valor + '</option>');
            });
            $('#objeto_objeto_3').html('<option value="-1">Seleccione una opción</option>');
            $.each(result.respuestas.objetos, function (key, value) {
                $("#objeto_objeto_3").append('<option value="' + value
                    .id + '">' + value.Valor + '</option>');
            });			
          ...
          if (result.datos) {
            $("#objeto_objeto_1").val(result.datos.OBJETO_1??-1).change();
            $("#objeto_desc_obj_1").val(result.datos.DESC_OBJ_1);
            $("#objeto_cant_obj_1").val(result.datos.CANT_OBJ_1);
            $("#objeto_valor_obj_1").val(result.datos.VALOR_OBJ_1);
            $("#objeto_estatus_obj_1").val(result.datos.ESTATUS_OBJ_1??-1);

            $("#objeto_objeto_2").val(result.datos.OBJETO_2??-1).change();
            $("#objeto_desc_obj_2").val(result.datos.DESC_OBJ_2);
            $("#objeto_cant_obj_2").val(result.datos.CANT_OBJ_2);
            $("#objeto_valor_obj_2").val(result.datos.VALOR_OBJ_2);
            $("#objeto_estatus_obj_2").val(result.datos.ESTATUS_OBJ_2??-1);

            $("#objeto_objeto_3").val(result.datos.OBJETO_3??-1).change();
            $("#objeto_desc_obj_3").val(result.datos.DESC_OBJ_3);
            $("#objeto_cant_obj_3").val(result.datos.CANT_OBJ_3);
            $("#objeto_valor_obj_3").val(result.datos.VALOR_OBJ_3);
